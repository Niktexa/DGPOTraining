...
#         # model 
#         # CRITICAL FIX: Discriminator should handle z-vector stripping internally
#         obs_shape = get_shape_from_obs_space(obs_space)
        
#         # Strip z-vector size from observation shape for the base network
#         actual_obs_shape = (obs_shape[0] - args.max_z,)
        
#         base = CNNBase if len(actual_obs_shape) == 3 else NoisyMLPBase
#         self.base = base(args, actual_obs_shape)  # Initialize with stripped size
...
class R_Discriminator(nn.Module):
    ...
    def evaluate_actions(self, obs, rnn_states, action, masks, available_mask=None, active_masks=None, isTrain=False):
        ...
        # CRITICAL FIX: Strip z-vector here before passing to base network
        obs_without_z = obs[:, self.max_z:]  # Remove first max_z elements
        
        actor_features = self.base(obs_without_z)  # Pass stripped observation
...
class R_in_Critic(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        # base = CNNBase if len(cent_obs_shape) == 3 else NoisyMLPBase
