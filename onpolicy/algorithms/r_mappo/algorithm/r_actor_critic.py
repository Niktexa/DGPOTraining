...
class R_Actor(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self.z_counter = {0: 0, 1: 0}
        self.action_debug_count = 0
    ...
    def forward(self, obs, rnn_states, masks, available_actions=None, deterministic=False):
        ...
        # if obs.shape[0] == 1:  # Only debug single batch to avoid spam
        #     print(f"ACTOR DEBUG - obs shape: {obs.shape}")
        #     print(f"ACTOR DEBUG - Z-vector: {obs[:,:self._max_z].detach().cpu().numpy()}")
        #     print(f"ACTOR DEBUG - Game obs sample: {obs[:,self._max_z:].detach().cpu().numpy().flatten()[:5]}")
        ...
        # if obs.shape[0] == 1:
        #     print(f"ACTOR DEBUG - Actor features norm: {torch.norm(actor_features).item():.3f}")
        #     print(f"ACTOR DEBUG - Z features norm: {torch.norm(z_features).item():.3f}")
