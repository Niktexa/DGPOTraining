...
class R_MAPPOPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu"), \
        ...
        self.ex_critic = R_in_Critic(args, self.share_obs_space, self.device)
    ...
    def evaluate_z(self, cent_obs, rnn_states_z, masks, active_masks=None, isTrain=False):
        # Extract z-vector from the FIRST max_z elements
        z_vec = cent_obs[:, :self.max_z]
        z_idx = np.argmax(z_vec, axis=1)
        z_idxs = np.expand_dims(z_idx, -1)
        
        # CRITICAL FIX: Don't strip z-vector here since discriminator expects full obs
        # The discriminator network was initialized with the full observation space
        cent_obs_for_discriminator = cent_obs  # Keep full observation
        
        available_mask = None
        
        action_log_probs, rnn_states_z = \
            self.discriminator.evaluate_actions(
                cent_obs_for_discriminator,  # Pass full observation
                rnn_states_z, 
                z_idxs, masks, 
                available_mask=available_mask, 
                active_masks=active_masks,
    ...
    def evaluate_local_z(self, obs, rnn_states_z, masks, active_masks=None):
        # Extract z-vector for skill classification but don't strip yet
        z_idx = np.argmax(obs[:, :self.max_z], axis=1)
        z_idxs = np.expand_dims(z_idx, -1)
        
        # CRITICAL FIX: Don't strip z-vector here since local_discri handles it internally
        # Pass the full observation to the local discriminator
        
        action_log_probs, rnn_states_z = \
            self.local_discri.evaluate_actions(
                obs,  # Pass full observation - local_discri will strip z internally
                rnn_states_z, 
                z_idxs, 
                masks, 
                active_masks=active_masks
