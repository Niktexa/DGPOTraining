...
class PongRunner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        # if total_num_steps % 1000 == 0:  # Every 1000 timesteps
        #     print(f"\nDEBUG Z-BEHAVIOR at timestep {total_num_steps}:")
        #     for z in range(min(self.max_z, 2)):  # Test first 2 z-values
        #         print(f"  Testing z={z}:")
        #         # Reset all eval environments with the same z
        #         test_obs = self.eval_envs.reset([z] * self.n_eval_rollout_threads)
        #         test_actions = []
                
        #         # Use proper tensor shapes matching your eval setup
        #         eval_rnn_states = np.zeros((self.n_eval_rollout_threads, *self.buffer.rnn_states.shape[2:]), dtype=np.float32)
        #         eval_masks = np.ones((self.n_eval_rollout_threads, self.num_agents, 1), dtype=np.float32)
        ...
        #                 deterministic=True
        #             )
        #             # Take first environment's action
        #             eval_actions = np.array(np.split(_t2n(eval_action), self.n_eval_rollout_threads))
        #             action_val = eval_actions[0, 0]  # First env, first agent
        #             test_actions.append(action_val)
                    
        #             # Step only first environment
        #             test_obs, _, _, _ = self.eval_envs.step(eval_actions)  # Fixed syntax
        #             eval_rnn_states = np.array(np.split(_t2n(eval_rnn_states), self.n_eval_rollout_threads))
                
        #         print(f"    Actions: {test_actions}")
        #         action_entropy = len(set(test_actions)) / len(test_actions)
        #         print(f"    Action diversity: {action_entropy:.2f}")
