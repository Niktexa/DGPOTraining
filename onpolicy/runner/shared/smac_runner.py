...
class SMACRunner(Runner):
    ...
    def run(self):
        ...
        print(f"📈 Starting DGPO training...")
        self.warmup()   
        ...
        for episode in range(episodes):   #core DGPO 
    ...
    def VMAPD_collect(self, step):
        ...
        flattened_obs = np.concatenate(self.buffer.share_obs[step+1]).flatten()
        
        # Reshape to (batch_size, obs_size) for the policy
        batch_size = self.n_rollout_threads * self.num_agents
        obs_size = flattened_obs.shape[0] // batch_size
        reshaped_obs = flattened_obs.reshape(batch_size, obs_size)
        ...
        # Also fix the local_z call
        flattened_local_obs = np.concatenate(self.buffer.obs[step+1]).flatten()
        reshaped_local_obs = flattened_local_obs.reshape(batch_size, obs_size)
